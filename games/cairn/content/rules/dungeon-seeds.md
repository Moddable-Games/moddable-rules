...
- **Purpose**: roll on `dungeon-purpose` (Original Use / Built By).
- **Construction**: roll on `dungeon-construction` (Entrance / Composition).
- **Ruination**: roll on `dungeon-ruination` (Condition / Cause).

#### Dungeon Denizens

Most dungeons will have creatures that _live_ there and make it their home. The `dungeon-denizen-traits` table (Virtues / Vices) can be used to describe the general attitude of any or all of the intelligent creatures in the dungeon. Roll on it, writing the results in your notebook. This describes the general attitude of the creatures found in the dungeon.
...
All dungeons should have one or more factions with their own unique agenda, obstacles, leaders, and hierarchy. Roll on `dungeon-faction-agendas` (Goal / Obstacle).
...
Roll on `dungeon-room-monster` (Group / Activity).
...
Roll on `dungeon-room-lore` (Room Type / Clue).
...
Roll on `dungeon-room-special` (Special / Feature).
