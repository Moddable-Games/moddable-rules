...
approximations:
  - feature: "What \"had turned clockwise\" means"
    source: "'When the King reaches the starting square of the opposite King, but only if had turned clockwise.' The page states the condition and never defines it."
    engine: "Implemented as net travel about the centre of the board: every King move adds the angle it sweeps, and the win requires that sum to be positive on arrival. Going the long way round counts; doubling back the short way does not."
    blocker: "Not a missing capability - a reading. Recorded because it is ours rather than the source's, and a different reading would change which games are won."
engine:
  topology:
    type: grid
    rows: 7
  ...
  plugins:
    chess:
      ...
      regions:
        ...
        # "When reaching the starting square of opposite Pawns it promotes."
        # White's pawns start on c1 and c2, Black's on e6 and e7, so each side
        # promotes on the other's two squares. Per seat, like the castles.
        # Each side's goal is the other side's King's starting square: White's
        # King starts on d2, Black's on d6.
        kingGoal:
          cells:
            - [[1, 3]]
            - [[5, 3]]
        promotionZone:
          cells:
            - [[1, 4], [0, 4]]
            - [[6, 2], [5, 2]]
      # Promotion is declared as what it is - a type, a zone, and what it may
      # become - because this pawn is not generated by the pawn machinery: its
      # forward direction turns with the ring.
      # "When the King reaches the starting square of the opposite King, but
      # only if had turned clockwise."
      goal:
        piece: king
        in: kingGoal
        rotation: cw
