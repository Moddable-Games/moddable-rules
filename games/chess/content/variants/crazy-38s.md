...
unsupported: "The 38-square shape is not in this file at all. `missing_squares: true` is a boolean, not a list of cells, and `setup` is the standard 64-square opening FEN, so the variant loads as ordinary 8x8 chess: 64 cells, 32 pieces, 20 opening moves, measured. It is not an unread key - `voids` is the supported way to declare a shaped board and is honoured in play, as romanchenkos-chess demonstrates. Blocked on content: chessvariants.com/38.dir/crazy/board.html carries the board as a diagram image and its text says only that the board has 38 squares and that not every letter-number combination appears, so which 26 are absent could not be recovered. Attribution checked and correct: Ben Good, for Hans Bodlaender's 38-challenge, page dated 1998-02-03. One claim in `special` is NOT supported by the source: the Rook page (38.dir/crazy/rook.html) says only that the Rook moves any number of squares orthogonally with an unblocked path, and mentions no loop connection at all, so the built-in loop this file credits to the Rook needs checking against the diagram before it is relied on."
engine:
  topology:
    type: grid
    cells: 38
