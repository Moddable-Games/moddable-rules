...
- Roll on `forest-trail-types` (Type / Marker) for each trail, recording the results in your notebook.
...
Roll on `forest-poi-monster` (Monster / Activity).
...
Roll on `forest-poi-ruins` (Ruin / Feature).
...
Roll on `forest-poi-shelter` (Shelter / Feature).
...
For decades, the order and stability of the forest was maintained through an agreement between the forest witch **Anglwyst** and **Sweeney**, the Wood's champion. The witch would use her magic to keep the trees and animals healthy, while the champion would ensure that hunters never kill any creature deemed essential to the Wood (indicated by a shock of white in their fur). Their pact was permanent and immalleable, and the forest was divided, barring both the witch and the champion from crossing its boundary.
...
Note: the source publishes an illustrative map image alongside this example; as an image asset it is not reproduced here.
...
- Shows the shortest path to the object of a heart's _true_ desire. Smells of lavender. Adds an extra Fatigue if read more than once per day. The process is cumulative.
...
**Shelter**: _Fishing Hut_, _Occupied by Army Deserters_
