...
Note: the source publishes an illustrative example map image alongside this chapter; as an image asset it is not reproduced here.
...
The most important aspect of a region is its _people_: their character, ambitions, and the resources that tie them to this land. Roll on `culture` and `resources`, then note the results in your notebook.
...
Roll one or more times on `faction-types`, `faction-traits`, `faction-advantages`, and `faction-agendas`.

##### Faction Advantages Table
...
- Whenever a faction is positioned to advance a goal in their agenda, roll on `faction-actions`.
...
2. Note the face-up result for each die, then use `terrain-die-drop` to mark the terrain type on the map. For example, a die showing a 4 would indicate Tough terrain.
3. Draw long, curvy lines across the page, carving up the map around each die.
4. Roll on the appropriate table for each result (`terrain-easy`, `terrain-tough`, or `terrain-perilous`), writing the terrain and landmark results on the map.
...
- Note the face-up result for each die, then use `poi-die-drop` to mark the type of POI on the map. For example, a die showing a 4 would indicate a Lair, denoted by the letter "L" on the page.
- Roll any dice that fell off the map page, repeating the last step until none remain.
...
Roll on `poi-settlements`, `poi-waypoints`, `poi-curiosities`, `poi-lairs`, or `poi-dungeons` as appropriate to the POI type rolled. See the [Dungeon Seeds](dungeon-seeds/) chapter for how to flesh out each Dungeon POI for your setting.
...
- Roll on `path-features` for each path, writing the results in your notebook.
